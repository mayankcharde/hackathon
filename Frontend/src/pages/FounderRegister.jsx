...
const FounderRegister = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // Register with the explicit 'founder' role
      await register(
        formData.name,
        formData.email,
        formData.password,
      ...
      // Redirect to the scanner so they can continue finding the item
      navigate("/scan");
    } catch (err) {
      ...
      <div className="page-container-narrow relative z-10">
        ...
        >
          ...
          <p className="text-center text-slate-400 mb-8">
            Authorize contact with owners and return items safely
          </p>

          {error && (
            <div className="mb-6 p-4 bg-red-500/10 border border-red-500/30 text-red-300 rounded-lg flex items-center space-x-2">
          ...
          <p className="mt-8 text-center text-slate-400 font-medium">
            ...
            <Link to="/founder-login" className="text-cyan-300 hover:underline">
              Sign In
            </Link>
          </p>
        </motion.div>
